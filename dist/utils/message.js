...
const messages = {
    general: {
        ...
        invalidFieldErr: (value, field, fieldRegex) => `${value} is invalid ${field}!\n${fieldRegex}`,
        alreadyExistsFieldErr: (value, field) => `${field} "${value}" already exists!`,
        fieldsRegex: {
            fullName: (minLength, maxLength) => `The field length has to be between ${minLength} and ${maxLength}`,
            email: "Email should follow the correct email format 'username@example.com'",
            password: "Password should follow the following criteria:\n" +
    ...
    users: {
        ...
        userCreatedSuccess: (fullName) => `Welcome to Agent Book ${fullName} :)`,
